...
alter table instituicao drop constraint instituicao_num_concelho_fkey;
alter table instituicao add constraint instituicao_num_concelho_fkey foreign key(num_concelho,num_regiao)
...
alter table prescricao drop constraint prescricao_num_cedula_fkey;
alter table prescricao add constraint prescricao_num_cedula_fkey foreign key(num_cedula, num_doente, data)
  references consulta(num_cedula, num_doente, data) on update cascade on delete cascade;

alter table analise drop constraint analise_num_cedula_fkey;
alter table analise add constraint analise_num_cedula_fkey foreign key(num_cedula, num_doente, data)
  references consulta(num_cedula, num_doente, data) on update cascade on delete cascade;
...
alter table prescricao_venda drop constraint prescricao_venda_num_cedula_fkey;
alter table prescricao_venda add constraint prescricao_venda_num_cedula_fkey foreign key(num_cedula, num_doente, data, substancia)
  references prescricao(num_cedula, num_doente, data, substancia) on update cascade on delete cascade;

UPDATE analise
  SET nome = 'glicemia' WHERE EXTRACT(YEAR from data)=2020;